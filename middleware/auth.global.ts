...
export default defineNuxtRouteMiddleware(async (to, from) => {
  ...
  if (process.server) {
    ...
    if (!REGEX_CRAWLER.test(userAgent)) {
      ...
      if (user && !to.query.id) {
        if (!isEditor && user.meta?.coloring?.current) {
          key = user.meta.coloring.current
        } else if (isEditor && user.meta?.coloring?.editor) {
          key = user.meta.coloring.editor
        } else if (isEditor) {
          key = ''
        }
      }
    }
    if (hasEditor && key.length) {
      fallbackEditor = es.loadFromCloud(key)
    }
  ...
  const fallbackHome = fetchHome ? Promise.all([
  ...
  const response = await Promise.all([fallbackEditor, fallbackHome])
  if (response[1] && response[1][0].data.value && response[1][1].data.value && response[1][2].data.value && response[1][3].data.value) {
    hs.setData([
      response[1][0].data.value,
      response[1][1].data.value,
      response[1][2].data.value,
